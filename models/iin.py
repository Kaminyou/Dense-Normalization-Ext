...
class PrefetchInterpolatedInstanceNorm(nn.Module):
    def __init__(self, out_channels=None, affine=True, device="cuda"):
        super(PrefetchInterpolatedInstanceNorm, self).__init__()
...
def init_prefetch_interpolated_instance_norm(
    model, y_anchor_num, x_anchor_num,
):
    for _, layer in model.named_modules():
        if isinstance(layer, PrefetchInterpolatedInstanceNorm):
            layer.init_collection(
                y_anchor_num=y_anchor_num, x_anchor_num=x_anchor_num
            )
